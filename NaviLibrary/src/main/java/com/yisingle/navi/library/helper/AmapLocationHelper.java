package com.yisingle.navi.library.helper;
...
public class AmapLocationHelper {
     ...
     */
    private static AMapLocationClientOption getDefaultOption(boolean isOnceLocation, int interval) {
        AMapLocationClientOption mOption = new AMapLocationClientOption();
        //可选，设置定位模式，可选的模式有高精度、仅设备、仅网络。默认为高精度模式
        mOption.setLocationMode(AMapLocationClientOption.AMapLocationMode.Hight_Accuracy);
        //可选，设置是否gps优先，只在高精度模式下有效。默认关闭
        mOption.setGpsFirst(false);
        //可选，设置网络请求超时时间。默认为5秒。在仅设备模式下无效
        mOption.setHttpTimeOut(5000);
        //可选，设置定位间隔。默认为2秒
        mOption.setInterval(interval);
        //可选，设置是否返回逆地理地址信息。默认是true
        mOption.setNeedAddress(true);
        //可选，设置是否单次定位。默认是false
        mOption.setOnceLocation(isOnceLocation);
    ...
    public static void startSingleLocate(Context context, final OnLocationGetListener onLocationGetListener) {
        ...
        //设置定位参数
        AMapLocationClientOption locationOption = getDefaultOption(true, 2000);
        locationClient.setLocationOption(locationOption);
